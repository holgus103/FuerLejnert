...
for set_num in range(0, len(sets)):
    ...
    for c_num in range(0, len(configs)):
        ...
        print(xlearn)
        print(xtest)
        s_laplacean = SVR(kernel ='precomputed').fit(kf.laplacean(xlearn, xlearn, configs[c_num][1]), rlearn)
        r_laplacean = s_laplacean.predict(kf.laplacean(xtest, xtest, configs[c_num][1]))
        r_sinc = SVR(kernel ='precomputed').fit(kf.sinc(xlearn, xlearn, configs[c_num][0]), rlearn).predict(kf.sinc(xtest, xtest, configs[c_num][0]))
        r_quadratic = SVR(kernel ='precomputed').fit(kf.quadratic(xlearn, xlearn, configs[c_num][0]), rlearn).predict(kf.quadratic(xtest, xtest, configs[c_num][0]))
        r_multiquadric = SVR(kernel ='precomputed').fit(kf.multiquadric(xlearn, xlearn, configs[c_num][0]), rlearn).predict(kf.multiquadric(xtest, xtest, configs[c_num][0]))
        r_rbf = svr_rbf.fit(xlearn, rlearn).predict(xtest)
